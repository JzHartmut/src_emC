...
Proxy2TargetSharedMem_Inspc* ctor_Proxy2TargetSharedMem_Inspc(ObjectJc* othiz, char const* nameAppl, ThCxt* _thCxt) {
  ...
  if(thiz->super.proxy2target !=null) {
    //initialize.
    setInt32BigEndian(&thiz->super.proxy2target->length_cmd, sizeof(TelgTarget2Proxy_Inspc_s) << 16);  //initialize
    setInt32BigEndian(&thiz->super.proxy2target->seqnr, 0);  //initialize
  }
  return thiz;
}
