...
//  ClassJc const refl_VaArgBuffer_Data = INIZ_ClassJc(refl_VaArgBuffer_Data, "VaArgBuffer_Data");
//  ClassJc const refl_VaArgBuffer = INIZ_ClassJc(refl_VaArgBuffer, "VaArgBuffer");
#endif
...
{ va_list src_args = src.args;
  ...
  { char* typeArgs = ythis->sType;  //The description to the content of ythis->data!
    ...
    if(typeArgs[ parNrString=(int)strcspn(typeArgs, "Sz") ] !=0)
      ...
      { char const* srcToCopy;
        ...
        { case 'C': *((char_va_list*)dst) = va_arg(src_args, char_va_list); dst += sizeof(char_va_list); break; 
          ...
          case 'z': { srcToCopy = va_arg(src_args, char const*); 
                      ...
                      nrofCharsToCopy = (int)strlen(srcToCopy);
                      dst = copyString_VaArgBuffer(ythis, dst, srcToCopy, nrofCharsToCopy, typeArgs); 
                    } break;
    ...
    { //No string argument in the list, it is the most standard case. 
      ...
      { ASSERT_emC(src.typeArgs == null || *src.typeArgs == 0, "", 0,0); //typeArgs should be empty also! All others is a software error. Exception warrantable.
      }
    }
  }
}
