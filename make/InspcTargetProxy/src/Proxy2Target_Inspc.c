...
int32 get_Proxy2Target_Inspc(Proxy2Target_Inspc* thiz, Cmd_InspcTargetProxy_e cmd, struct RemoteAddressJc* address, int32 input) {
  ...
  if(timeout < 0){
    ...
    printf("%8.8x %s-@%8.8x x=%8.8x ret=%8.8x\n", (int)cmd, cmdTxt[((int)cmd) & 0x1f], (int32)address, input, value);
  }
  return value; 
